...
public class WearhouseAutoRed extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
       ...
       // drive.updateSlideLength = false;
        ...
        drive.followTrajectory(this,
                new Trajectory(new TrajectoryPeice(drive.currentPose, 0, 0.5), true)
                        .addLine(new TrajectoryPeice(new Pose2d(42, endPoint.y, 0), 0, 0.95),true),
                true
        );
    ...
    public void driveIn(Pose2d endPoint){
        ...
        if (angle != 0) {
            drive.followTrajectory(this,
                    new Trajectory(new TrajectoryPeice(drive.currentPose, 0, 0.5), true)
                            .addLine(new TrajectoryPeice(new Pose2d(x - 5, endPoint.y, 0), 0, 0.95),true)
                            .addLine(new TrajectoryPeice(new Pose2d(x, y, angle), 0, 0.6),true)
                            .end()
            );
        }
        else{
            drive.followTrajectory(this,
                    new Trajectory(new TrajectoryPeice(drive.currentPose, 0, 0.5), true)
                            .addLine(new TrajectoryPeice(new Pose2d(x, endPoint.y, 0), 0, 0.95),true)
                            .end()
            );
            int k = 6;
            if (numMinerals == 0){
    ...
    public void driveOut(Pose2d endPoint){
        ...
        drive.followTrajectory(this,
                new Trajectory(new TrajectoryPeice(drive.currentPose, 0, 0.5), true)
                        .addLine(new TrajectoryPeice(new Pose2d(36.5, newEnd.getY() + side * 0.2, 0), 0, 0.85))
                        .addLine(new TrajectoryPeice(new Pose2d(newEnd.getX() + 3.5,newEnd.getY() + 0.25 * side, 0), 0, 0.6))
                        .end()
        );
